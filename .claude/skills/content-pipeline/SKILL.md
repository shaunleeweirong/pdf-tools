...
3. **Draft new topics.** Pick the top 1 to 2 `status: "idea"` topics by `priority` not covered by a published slug. For each: run `content-research` to a brief, then `content-draft` to a finished `content/pending/<slug>.mdx` (≥800 words; follow content-draft's style **and E-E-A-T** rules: no em-dashes, no AI-tell words, 1 to 3 credible outbound links, `faq:` frontmatter, and **every claim about our own tools traced to `docs/marketing/tool-measurements.md`**, never estimated) with the topic set to `pending` + `slug`. Research-first: no draft without a brief.
